        // ================= KPSS TAKİBİ =================
        let kpssSubjects = []; // {id, name}
        let kpssSubjectIdCounter = 1;
        let kpssTopics = []; // {id, subjectId, name}
        let kpssTopicIdCounter = 1;
        let kpssUnits = []; // {id, topicId, name, techniques[11], questions[], saved, showTechniques, quizMode, quizResult}
        ...
        function removeKpssSubject(id) {
            if (!confirm('Bu ders ve içindeki tüm konular/ders birimleri silinecek. Emin misin?')) return;
            const topicIds = kpssTopics.filter(t => t.subjectId === id).map(t => t.id);
            kpssUnits = kpssUnits.filter(u => !topicIds.includes(u.topicId));
            kpssTopics = kpssTopics.filter(t => t.subjectId !== id);
            kpssSubjects = kpssSubjects.filter(s => s.id !== id);
            renderKpss();
        }
        ...
        function removeKpssTopic(id) {
            if (!confirm('Bu konu ve içindeki tüm ders birimleri silinecek. Emin misin?')) return;
            kpssUnits = kpssUnits.filter(u => u.topicId !== id);
            kpssTopics = kpssTopics.filter(t => t.id !== id);
            renderKpss();
        }

        function addKpssUnit(topicId) {
            const input = document.getElementById('new-kpss-unit-' + topicId);
            const name = input.value.trim();
            if (!name) { alert('Ders birimi adı boş olamaz.'); return; }
            kpssUnits.push({
                id: kpssUnitIdCounter++,
                topicId,
                name,
        ...
        function removeKpssUnit(id) {
            kpssUnits = kpssUnits.filter(u => u.id !== id);
            renderKpss();
        }

        function harvestKpssUnitForm(unitId) {
        ...
        function parseKpssQuestionText(unitId) {
        ...
        function parseKpssTechniqueText(unitId) {
        ...
        function selectKpssAnswer(unitId, questionId, optionIndex) {
            if (!kpssQuizAnswers[unitId]) kpssQuizAnswers[unitId] = {};
            kpssQuizAnswers[unitId][questionId] = optionIndex;
            renderKpss();
        }

        function finishKpssQuiz(unitId) {
            const unit = kpssUnits.find(u => u.id === unitId);
            if (!unit) return;
            const answers = kpssQuizAnswers[unitId] || {};
        ...
        function renderKpssQuiz(unit) {
            const answers = kpssQuizAnswers[unit.id] || {};
            ...
            const questionsHtml = validQuestions.map((q, idx) => {
                const optsHtml = letters.map((letter, o) => {
                    ...
                    return `<label style="display:flex; align-items:center; gap:6px; font-size:0.8rem; margin-top:3px; cursor:pointer;">
                        <input type="radio" name="quiz-${unit.id}-${q.id}" ${selected ? 'checked' : ''} onchange="selectKpssAnswer(${unit.id}, ${q.id}, ${o})">
            ...
            <div style="margin-top:12px; border:1px solid var(--accent-gold); border-radius:8px; padding:12px;">
                ...
                <div style="display:flex; gap:8px; margin-top:10px;">
                    <button class="btn-action btn-primary" onclick="finishKpssQuiz(${unit.id})">Bitirdim, Sonucu Gör</button>
                    <button class="btn-action" onclick="closeKpssQuiz(${unit.id})">Kapat</button>
                </div>
            </div>`;
        }

        function renderKpssUnitEditForm(unit) {
            ...
            const questionsHtml = unit.questions.map((q, idx) => `
                <div style="border:1px solid var(--border-color); border-radius:6px; padding:10px; margin-bottom:10px;">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        ...
                        <button class="btn-action" style="color:var(--accent-red); padding:2px 8px;" onclick="removeKpssQuestion(${unit.id}, ${q.id})">✕</button>
                    </div>
            ...
            <div class="prayer-card">
                ...
                <div style="margin:10px 0; padding:10px; border:1px dashed var(--accent-blue); border-radius:6px;">
                    ...
                    <button class="btn-action btn-primary" style="margin-top:6px;" onclick="parseKpssTechniqueText(${unit.id})">Ayrıştır ve Doldur</button>
                </div>
                ...
                <div style="margin:10px 0; padding:10px; border:1px dashed var(--accent-gold); border-radius:6px;">
                    ...
                    <button class="btn-action btn-primary" style="margin-top:6px;" onclick="parseKpssQuestionText(${unit.id})">Ayrıştır ve Ekle</button>
                </div>
                <button class="btn-action" onclick="addKpssQuestion(${unit.id})">+ Soru Ekle</button>
                <button class="btn-action btn-primary" style="margin-top:10px;" onclick="saveKpssUnit(${unit.id})">Kaydet</button>
            </div>`;
        }
            ...
            <div class="prayer-card">
                <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px;">
                    ...
                    <div style="display:flex; gap:6px; flex-wrap:wrap;">
                        ...
                        <button class="btn-action" onclick="editKpssUnit(${unit.id})">Düzenle</button>
                        <button class="btn-action" style="color:var(--accent-red)" onclick="removeKpssUnit(${unit.id})">Sil</button>
                    </div>
                </div>
        ...
        function renderKpssUnitCard(unit) {
            return unit.saved ? renderKpssUnitSummary(unit) : renderKpssUnitEditForm(unit);
        }
        ...
        function renderKpssTopicCard(topic) {
            ...
            <div class="prayer-card" style="border-color:var(--accent-blue);">
                ...
                <div class="form-row">
                    <input type="text" id="new-kpss-unit-${topic.id}" placeholder="Ders birimi adı (örn: 1. Ders)">
                    <button class="btn-action btn-primary" onclick="addKpssUnit(${topic.id})">+ Ders Birimi Ekle</button>
                </div>
                <div style="display:flex; flex-direction:column; gap:10px;">
                    ${units.length ? units.map(renderKpssUnitCard).join('') : '<span style="color:var(--text-muted); font-size:0.8rem;">Henüz ders birimi eklenmedi.</span>'}
                </div>
            </div>`;
        }
        ...
        function renderKpss() {
            ...
            document.getElementById('kpss-count').innerText = kpssSubjects.length + ' ders';
        }
